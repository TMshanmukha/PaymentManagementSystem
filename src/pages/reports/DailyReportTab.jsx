    ...
    <div>
      ...
      {data && (
        <div className="print-area print-a4">
          ...
          {(() => {
            const accountantsList = data.accountantBreakdown.filter(a => a.userRole !== 'ADMIN');
            const adminsList = data.accountantBreakdown.filter(a => a.userRole === 'ADMIN');
            return (
              <>
                <Card className="mb-4">
                  <p className="font-semibold text-navy-900 mb-3">Accountant-wise Breakdown</p>
                  {accountantsList.length === 0 ? (
                    <p className="text-sm text-slate-400 py-4">No accountant activity recorded for this date.</p>
                  ) : (
                      ...
                      {accountantsList.map((a) => (
                        <div key={a.userId} className="border border-slate-100 rounded-lg p-3 hover:shadow-sm transition-all bg-white">
                          ...
                          <div className="mt-3 pt-2.5 border-t border-slate-100 flex justify-between items-center text-xs">
                            <span className="text-slate-500 font-medium">Net Collection:</span>
                ...
                <Card className="mb-4">
                  <p className="font-semibold text-navy-900 mb-3">Co-Admin-wise Breakdown</p>
                  {adminsList.length === 0 ? (
                    <p className="text-sm text-slate-400 py-4">No co-admin activity recorded for this date.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {adminsList.map((a) => (
                        <div key={a.userId} className="border border-slate-100 rounded-lg p-3 hover:shadow-sm transition-all bg-white">
